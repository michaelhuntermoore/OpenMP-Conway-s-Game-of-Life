...
void runGame(PPMImage *inputImage) {
	...
	int Col = inputImage->x;
	int Row = inputImage->y;
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < Row; i++) {
		for (j = 0; j < Col; j++) {
			...
			if ((int)inputImage->data[j*i].red > 0 && ((int)inputImage->data[j*i].green > 0) && ((int)inputImage->data[j*i].blue > 0)) {
				...
				cellMatrix[1][1] = inputImage->data[j * i].red;
				...
				if (numLiveCells == 1 || numLiveCells == 2) {
					inputImage->data[j*i].red = 0;
					inputImage->data[j*i].green = 0;
					inputImage->data[j*i].blue = 0;
				}
				...
				//	if (cellMatrix[1][2] == 255)
				//		cout << "Live E neighbor at " << E << "\n";
				//	if(cellMatrix[2][0] == 255)
				//		cout << "Live SW neighbor at " << SW << "\n";
				//	if (cellMatrix[2][1] == 255)
				//		cout << "Live S neighbor at " << SW << "\n";
				//	if (cellMatrix[2][2] == 255)
				//		cout << "Live SE neighbor at " << SW << "\n";
				//-----------------------------------------------------------------//

			}
			else if ((int)inputImage->data[j*i].red == 0) {
				//Get the neighborhood values for a target live cell.
				NW = abs((i - 1) * (j - 1));
				N = abs((i - 1) * j);
				NE = abs((i - 1) * (j + 1));
				...
				if (numLiveCells == 3) {
					inputImage->data[j*i].red = 255;
					inputImage->data[j*i].green = 255;
					inputImage->data[j*i].blue = 255;
				}
			}
